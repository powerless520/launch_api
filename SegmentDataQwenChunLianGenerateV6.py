...
class ChunlianGenerator:
    ...
    def create_duilian(self, text_images, save_name):
        ...
        if save_name == '上联':
            save_path = os.path.join(RESULT_PATH,
                                     self.formatted_time + '-' + self.custom_text + '_' + save_name + '_' + self.shanglian + '.png')
        else:
            save_path = os.path.join(RESULT_PATH,
                                     self.formatted_time + '-' + self.custom_text + '_' + save_name + '_' + self.xialian + '.png')

        image_height = self.image_height * len(text_images)
        image_width = self.image_width

        spring_festival_image = Image.new('RGB', (image_width + 40, image_height + 30), color=(114, 20, 0))
        ...
        for i, text_image in enumerate(text_images):
            # 计算贴上文字图片的位置
            position = 20 + (image_width - text_image.width) // 2, i * text_height + 100 // len(text_images)
            # 将0像素转为透明
            text_image = self.make_transparent(text_image)
    ...
    def create_hengpi(self, text_images, save_name):
        ...
        save_path = os.path.join(RESULT_PATH,
                                 self.formatted_time + '-' + self.custom_text + '_' + save_name + '_' + self.hengpi + '.png')
    ...
    def merg_chunlian(self):
        ...
        height = horizontal_image.height + self.shanglian_image.height + 100 + 100 + 40
        canvas = Image.new("RGBA", (1284, height), (0, 0, 0, 0))

        # 计算上联图像的位置
        x1 = int(1284 / 2 - horizontal_image.width / 2)
        y1 = 350
        x2 = x1 + image1.width
        y2 = y1 + image1.height

        # 计算下联图像的位置
        x3 = 1284 - image2.width - int(1284 / 2 - horizontal_image.width / 2)
        y3 = 350
        x4 = x3 + image2.width
        y4 = y3 + image2.height
        ...
        self.chunlian = canvas
        # 保存结果图像
        # print()
        # save_path = os.path.join(RESULT_PATH,
        #                          self.formatted_time + '-' + self.custom_text + '-春联' + '-' + self.hengpi + '.png')
        save_path = os.path.join(RESULT_PATH,
                                 self.uuid + '-' + str(self.time_unix) + '.png')
        canvas.save(save_path)
        # returnPath = self.uuid + '-' + str(self.time_unix) + '.png'
        return save_path

    def merg_result(self):
        save_path = os.path.join(RESULT_PATH,
                                 self.formatted_time + '-' + self.custom_text + '-输出' + '-' + self.hengpi + '.png')
        # canvas = Image.new("RGBA", (1284+400, 1570), (0, 0, 0, 0))

        # height = self.hengpi_image.height+ self.shanglian_image.height+100+100+40
        height = 1024
        chunlian = self.chunlian
        # 输入的宽度
        new_height = height

        # 计算比例
        width, height = chunlian.size
        scale = new_height / height
        new_width = int(width * scale)

        # 对图片进行缩放

        chunlian = chunlian.resize((new_width, new_height))
        chunlian.save('test.png')
        ...
        x1 = 400
        y1 = 0
        x2 = x1 + chunlian.width
        y2 = chunlian.height
        canvas.paste(chunlian, (x1, y1, x2, y2), mask=chunlian)
        font1 = ImageFont.truetype("font/京華老宋体v1.007.ttf", size=70)
        font2 = ImageFont.truetype("font/京華老宋体v1.007.ttf", size=60)
        # 创建绘图对象
    ...
    def heng2shu(self, draw, position, font, text_color):
        # 要写的三句春联文本（每句7个字）
        texts = [self.hengpi, self.shanglian, self.xialian]

        # 初始的x和y坐标
        x_text, y_text = position

        # 逐句逐个字符绘制（垂直方向）
        for text in texts:
            for char in text:
                draw.text((x_text, y_text), char, font=font, fill=text_color)
                y_text += 60  # 假设每个字符的高度约为20像素
            x_text += 110  # 更新x坐标，移动到下一列
            y_text = position[1]  # 重置y坐标到顶部
...
def pipeline(custom_text):
    try:
        ...
        # print('DATA_PATH:' + DATA_PATH + "|" + 'RESULT_PATH:' + RESULT_PATH)
        ...
        for i in range(10):
            ...
            try:
                ...
                if len(shanglian) != len(xialian) or (custom_text in shanglian + xialian + hengpi) or len(
                        hengpi) != 4 or len(shanglian) > 9:
                    continue
                else:
